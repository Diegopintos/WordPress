<?php
/**
 * WPMovieLibrary-Details-Zone
 *
 * @package   WPMovieLibrary-Details-Zone
 * @author    lesurfeur
 * @license   GPL-3.0
...
if ( ! class_exists( 'WPMovieLibrary_Details_Zone' ) ) :
	...
	*/
	class WPMovieLibrary_Details_Zone extends WPMOLY_Details_Module {
		...
		public function init() {
			...
			if ( ! wpmoly_details_zone_requirements_met() ) {
				add_action( 'init', 'wpmoly_details_zone_l10n' );
				add_action( 'admin_notices', 'wpmoly_details_zone_requirements_error' );
				return false;
			}
			...
			$this->detail = array(
				'id'       => 'wpmoly-movie-zone',
				'name'     => 'wpmoly_details[zone]',
				'type'     => 'select',
				'title'    => __( 'Zone', 'wpmovielibrary-details' ),
				'desc'     => __( 'Select an zone for this movie', 'wpmovielibrary-details' ),
				'icon'     => 'dashicons dashicons-admin-site',
				'options'  => array(
					'zone0'		=> __( 'Zone 0', 'wpmovielibrary-details' ),
					'zone1'		=> __( 'Zone 1', 'wpmovielibrary-details' ),
					'zone2'		=> __( 'Zone 2', 'wpmovielibrary-details' ),
					'zone3'		=> __( 'Zone 3', 'wpmovielibrary-details' ),
					'zone4'		=> __( 'Zone 4', 'wpmovielibrary-details' ),
					'zone5'		=> __( 'Zone 5', 'wpmovielibrary-details' ),
					'zone6'		=> __( 'Zone 6', 'wpmovielibrary-details' ),
					'zonea'		=> __( 'Zone A', 'wpmovielibrary-details' ),
					'zoneb'		=> __( 'Zone B', 'wpmovielibrary-details' ),
					'zonec'		=> __( 'Zone C', 'wpmovielibrary-details' )
				),
				'default'  => 'Zone 2',
				'multi'    => true,
				'rewrite'  => array( 'zone' => __( 'zone', 'wpmovielibrary-details' ) )
			);
		}
		...
		public function register_hook_callbacks() {
			...
			add_action( 'plugins_loaded', 'wpmoly_details_zone_l10n' );
			...
			add_filter( 'wpmoly_format_movie_zone', array( $this, 'format_detail' ), 10, 1 );
		}
		 ...
		 */
		private function wpmoly_details_zone_requirements_error() {
		...
		public static function require_wpmoly_first() {
			...
			$this_plugin      = basename( $this_plugin_path ) . '/wpmoly-details-zone.php';
		...
		public function create_detail( $details ) {
			...
			$details = array_merge( $details, array( 'zone' => $this->detail ) );
		...
		public function detail_setting( $field ) {
			...
			$field['options']['available'] = array_merge( $field['options']['available'], array( 'zone' => $this->detail['title'] ) );
		...
		public function format_detail( $data, $format = 'html' ) {
			...
			$data = WPMovieLibrary::render_template( $view, array( 'detail' => 'zone', 'data' => 'zone', 'title' => implode( ', ', $title ) ), $require = 'always' );
